(function () {
    ...
    const IDLE_TIMEOUT_MS = 10800000;
    const PROMPT_TIMEOUT_MS = 60000;
    const MODAL_ID = 'jf-sleep-timer-modal';
    ...
    const ensureStyles = () => {
        if (document.getElementById('jf-sleep-style')) return;
        const style = document.createElement('style');
        style.id = 'jf-sleep-style';
        style.textContent = `
            #${MODAL_ID} {
                position: fixed;
                top: 0; 
    ...
    const createModal = () => {
        ...
        title.className = 'jf-sleep-title';
        title.textContent = 'Are you still watching?';

        const btn = document.createElement('button');
        btn.className = 'jf-sleep-btn';
    ...
    const exitPlayer = () => {
        const backBtn = document.querySelector('.btnHeaderBack') || document.querySelector('.videoOsdBack');
        if (backBtn) {
            backBtn.click();
        } else {
            window.history.back();
    ...
    const checkPlayerState = () => {
        ...
        if (video !== lastVideoRef) {
            ...
            if (video) {
                ...
                if (idleTimer) clearTimeout(idleTimer);
                if (isPromptActive) dismissPrompt();
            }
        }
    };
